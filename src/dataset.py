...
def prepare_dataloaders(config: Data) -> tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    train_data = KWDataset(data_path=config.path+"/train.json")
    val_data = KWDataset(data_path=config.path+"/val.json")
    test_data = KWDataset(data_path=config.path+"/test.json")
